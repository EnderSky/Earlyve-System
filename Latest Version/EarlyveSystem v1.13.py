...
filename = "Assets/Data Log.xlsx"          #Filename of file which logs down the data
...
class MainFrame ( wx.Frame ):
        ...
        def __init__( self, parent ):
                wx.Frame.__init__ ( self, parent, id = wx.ID_ANY, title = u"Earlyve System", pos = wx.DefaultPosition, size = wx.Size( 1000,700 ), style = wx.DEFAULT_FRAME_STYLE|wx.TAB_TRAVERSAL )
        ...
        def WriteEmail(self, filename, student_id, now, reason):        #Now is the variable containing the current datetime
                with open(filename, encoding='utf-8-sig') as csv_file:
                        ...
                        for row in csv_reader:
                                if row["Student ID"].upper() == student_id:
                                        selected_row = row
        ...
        def SendEmail(self, to_address, text, name):
                ...
                msg['Subject'] = "Student {name} {var_text}.".format(name=name, var_text=var_text)
        ...
        def OnSubmit( self, event ):
                ...
                nric = self.textfieldStudentID.GetValue().upper() #Get uppercase for standardisation
                ...
                with open("Assets/Student Database.csv", encoding='utf-8-sig') as csv_file:
                        ...
                        for row in csv_reader:
                                if row["Student ID"].upper() == nric:
                                        selected_row = row
                                    
                student_name = selected_row["Name"]
                student_class = selected_row["Class"]
                #Censor Student ID when displaying on screen
                censored_id = "*****{censored_nric}".format(censored_nric = nric[-4:])
        ...
        def OnQueryID( self, event ):
                ...
                if self.textfieldStudentID.GetValue() == "":
                        ...
                        with open("Assets/Student Database.csv", encoding='utf-8-sig') as csv_file:
                                ...
                                for row in csv_reader:
                                        if row["Student ID"].upper() == nric.upper(): #Convert both to uppercase for standardisation
                                                selected_row = row

                        #If selected row remains blank, means that the program did not manage to locate a student in the database                    
                        if len(selected_row) == 0:
                                self.textConfirmation.SetLabel("Student not found. Please try again.")
                                self.textEmailConfirmation.SetLabel("")
                                return

                        nric = selected_row["Student ID"]
                        student_name = selected_row["Name"]
                        student_class = selected_row["Class"]
                        ...
                        for row in range(1, empty):
                                ...
                                if sheet['B'+str(row)].value.upper() == nric.upper(): #Convert both to uppercase for standardisation
                                        
                                        #Append data entry to a list
                                        # Data Format: Date & Time      (Reason: ____)
                                        element = "{datetime}        (Reason: {reason})".format(datetime=sheet['A'+str(row)].value,\
                                                                                                 reason=sheet['E'+str(row)].value)
                                        entries.append(element)

                        #6. Print out list
                        #If list is empty, then no data entry recorded
                        if len(entries) == 0:
                                self.textConfirmation.SetLabel("Specified student has no entries. Please try again.")
                                self.textEmailConfirmation.SetLabel("")
                        #Else print out all the entries
                        else:
                                if self.mode == "EarlyLeave":
                                        text = "Student {name}, class {student_class} (ID: {nric}) \nhas taken early leave from school on these dates:\n".format(name=student_name,\
                                                                                                                                                                 student_class=student_class,\
                                                                                                                                                                 nric=nric)
                                elif self.mode == "Latecomers":
                                        text = "Student {student_name}, class {student_class} (ID: {nric}) \n\has been late for school on these dates:\n".format(name=student_name,\
                                                                                                                                                                 student_class=student_class,\
                                                                                                                                                                 nric=nric)
                                for item in entries:
                ...
                if self.textfieldStudentID.GetValue() == "":
                        ...
                        for row in range(2, empty):
                                ...
                                if curr_date == requested_date:
                                        #Append data entry to a list
                                        lyst = []
                                        lyst.append(sheet['B'+str(row)].value)  #ID
                                        lyst.append(sheet['C'+str(row)].value)  #Name  
                                        lyst.append(sheet['D'+str(row)].value)  #Class
                                        lyst.append(sheet['E'+str(row)].value)  #Reason
                                        entries.append(lyst)
                        ...
                        #Else print out all the entries
                                ...
                                if self.mode == "EarlyLeave":
                                        text = "Students who took early leave on {date}:\n".format(date=requested_date)
                                elif self.mode == "Latecomers":
                                        text = "Students who were late on {date}:\n".format(date=requested_date)
                                for item in entries:
                                        #Format of text: - Name, Class (ID: ____)        (Reason: _______)
                                        text += "- {name}, class {student_class} (ID: {nric})        (Reason: {reason})\n".format(name=str(item[1]),\
                                                                                                                                   student_class=str(item[2]),\
                                                                                                                                   nric=str(item[0]),\
                                                                                                                                   reason=str(item[3]))
                                self.textConfirmation.SetLabel("Data has been retrieved successfully.")
        ...
        def OnQueryName( self, event ):
                ...
                if self.textfieldStudentID.GetValue() == "":
                        ...
                        for row in range(2, empty):
                                ...
                                if requested_name.upper() in curr_name.upper(): #Convert both to uppercase for standardisation
                                        ...
                                        for curr_row in entries:
                                                ...
                                                if curr_row[0] == curr_name:
                                                        ...
                                                        element = "{datetime}        (Reason: {reason})".format(datetime=sheet['A'+str(row)].value,\
                                                                                                               reason=sheet['E'+str(row)].value)
                                                        curr_row.append(element)  #Date & Time
                                                        break
                                                ...
                                                element = "{datetime}        (Reason: {reason})".format(datetime=sheet['A'+str(row)].value,\
                                                                                                       reason=sheet['E'+str(row)].value)
                                                lyst = []
                                                lyst.append(curr_name) #Name
                                                lyst.append(sheet['D'+str(row)].value) #Class
                                                lyst.append(element) #Date & Time, Reason
                                                entries.append(lyst)  #Name, Class, Date & Time   (Reason: _______)
                        ...
                        #Else print out all the entries
                                ...
                                if self.mode == "EarlyLeave":
                                        text = "Students who contain the name '{name}' and took early leave:\n".format(name=requested_name)
                                elif self.mode == "Latecomers":
                                        text = "Students who contain the name '{name}' and were late for school:\n".format(name=requested_name)
                                for row in entries:
                                        text += "\n{name}, class {student_class}:\n".format(name=row[0], student_class=row[1])  #Add header for each unique student
                                        for item in range(2, len(row)):
                                                text += "- {entry}\n".format(entry=str(row[item])) #Add individual date & time data entries under each student's header
                                self.textConfirmation.SetLabel("Data has been retrieved successfully.")
